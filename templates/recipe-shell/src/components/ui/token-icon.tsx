...
export function TokenIcon({ symbol, logoURI, size = "md", className }: TokenIconProps) {
  ...
  return (
    <div className={cn("rounded-full overflow-hidden flex-shrink-0 bg-muted relative", sizeMap[size], className)}>
      <img
        src={logoURI}
        alt={symbol}
        className={cn("w-full h-full object-contain transition-opacity duration-150", isLoaded ? "opacity-100" : "opacity-0")}
